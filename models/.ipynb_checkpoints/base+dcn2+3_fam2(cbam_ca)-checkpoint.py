...
class SFconv(nn.Module):
    def __init__(self, features, r=2, L=24) -> None:
        ...
        self.convll = nn.Sequential(
            nn.Conv2d(features, features // 4, kernel_size=3, stride=1, padding=1, padding_mode='reflect'),
            nn.GELU(),
            nn.Conv2d(features // 4, features, kernel_size=3, stride=1, padding=1, padding_mode='reflect'),
            nn.Sigmoid(),
        )
    ...
    def forward(self, x):
        ...
        emerge1 = self.gap(self.norm(emerge1))
        #emerge1 = self.fc(emerge1)

        high_att = self.softmax(self.fcs(emerge1 ))
        middle_att = self.softmax2( self.fcs2(emerge1 ))

        fea_high = high_att * lowlow
        fea_middle = middle_att * low

        max_out,_=torch.max(emerge2,dim=1,keepdim=True)
        avg_out=torch.mean(emerge2,dim=1,keepdim=True)
        spa_out=self.sigmod(self.cov7(torch.cat([max_out,avg_out],dim=1)))

        emerge2 = spa_out*emerge2

        out = self.out(fea_high + fea_middle + emerge2)
