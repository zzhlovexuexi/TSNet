...
class SFconv(nn.Module):
    def __init__(self, features, r=2, L=24) -> None:
        ...
        self.fcs = nn.Sequential(
            nn.Conv2d(3*features, 3*features, 1, 1, 0),
            nn.GELU(),
            nn.Conv2d(3*features,3* features, 1, 1, 0)
        )
        ...
        self.out = nn.Sequential(
            nn.Conv2d(features, features , 1, padding=0, bias=True),
            nn.Sigmoid()
        )
        self.norm = nn.BatchNorm2d(features)
        self.norm1 = nn.BatchNorm2d(features)
        self.mlp=nn.Sequential(
            nn.Conv2d(features*3, features , 1, padding=0, bias=True),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        emerge1 = torch.cat([low , lowlow,middle],dim=1)

        emerge2 = self.gap(emerge1)
        emerge2 = self.softmax(self.fcs(emerge2))

        emerge2= emerge2 * emerge1
        ...
        fea_high2= spa_out*emerge2

        fea_high2=self.mlp(fea_high2)

        return fea_high2 + self.mix(x)
