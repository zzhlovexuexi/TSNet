...
class SFconv(nn.Module):
    ...
    def forward(self, x):
        ...
        emerge1 = spa_out*emerge1

        emerge2 = self.gap(emerge2)
        #emerge1 = self.fc(emerge1)

        high_att = self.softmax(self.fcs(emerge2 ))
        middle_att = self.softmax2( self.fcs2(emerge2 ))

        fea_high = high_att * middle
        fea_middle = middle_att * high

        out = self.out(fea_high + fea_middle + emerge1)
