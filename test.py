...
parser.add_argument('--model', default='TSNet-S', type=str, help='model name')
...
def test(test_loader, network,network2 ,result_dir):
	PSNR = AverageMeter()
	SSIM = AverageMeter()
	...
	for idx, batch in enumerate(test_loader):
		...
		with torch.no_grad():
			...
			qingxi = (input + output)
			qingxi=network2(qingxi).clamp_(-1, 1)

			# [-1, 1] to [0, 1]
			qingxi = qingxi * 0.5 + 0.5
			target = target * 0.5 + 0.5

			psnr_val = 10 * torch.log10(1 / F.mse_loss(qingxi, target)).item()

			_, _, H, W = qingxi.size()
			down_ratio = max(1, round(min(H, W) / 256))  # Zhou Wang
			ssim_val = ssim(F.adaptive_avg_pool2d(qingxi, (int(H / down_ratio), int(W / down_ratio))),
		...
		out_img = chw_to_hwc(qingxi.detach().cpu().squeeze(0).numpy())
...
if __name__ == '__main__':
	...
	saved_model_dir2 = os.path.join(args.save_dir, args.exp, args.model + '+ mix2' + '.pth')


	if os.path.exists(saved_model_dir) and os.path.exists(saved_model_dir2):
		print('==> Start testing, current model name: ' + args.model)
		network.load_state_dict(single(saved_model_dir))
		network2.load_state_dict(single(saved_model_dir2))
	else:
...
# 	macs2, params2 = get_model_complexity_info(network2, (3, 256, 256), as_strings=True,
# 											 print_per_layer_stat=True, verbose=True)
# 	print('{:<30}  {:<8}'.format('Computational complexity: ', macs2))
# 	print('{:<30}  {:<8}'.format('Number of parameters: ', params2))
# 	print('{:<30}  {:<8}'.format('Computational complexity: ', macs+'  '+macs2))
# 	print('{:<30}  {:<8}'.format('Number of parameters: ', params+'  '+params2))
	...
	test(test_loader, network, network2,result_dir)
